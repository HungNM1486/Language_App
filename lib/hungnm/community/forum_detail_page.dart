...
class _ForumDetailPageState extends State<ForumDetailPage> {
  ...
  void initState() {
    ...
    _post = widget.post;
    _comments = [
      {'user': 'Ngọc Lan', 'text': 'Bài viết rất hữu ích, cảm ơn bạn!', 'time': DateTime.now().subtract(const Duration(minutes: 30)), 'helpful': 5},
      {'user': 'Minh Tuấn', 'text': 'Mình cũng áp dụng cách này, hiệu quả lắm!', 'time': DateTime.now().subtract(const Duration(hours: 1)), 'helpful': 2},
    ];
  }
          ...
          children: [
            ...
            Padding(
              ...
              child: Row(
                children: [
                  GestureDetector(
                    onTap: () => _navigateToUserProfile('author-${_post.id}', _post.authorName),
                    child: CircleAvatar(
                      radius: 20,
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(timeago.format(_post.postedTime, locale: 'vi'), style: TextStyle(color: Colors.grey.shade600)),
                      ],
                    ),
                  ),
                ],
                  ...
                  itemBuilder: (context, index) => GestureDetector(
                    onTap: () {
                      showDialog(
                        context: context,
                        builder: (_) => Dialog(
                          child: CachedNetworkImage(imageUrl: _post.imageUrls[index], fit: BoxFit.contain),
                        ),
                      );
                    },
                    child: Padding(
                      padding: const EdgeInsets.all(8),
                      child: CachedNetworkImage(imageUrl: _post.imageUrls[index], width: 200, fit: BoxFit.cover),
                    ),
                  ),
                ),
              ),
            ...
            Padding(
              ...
              child: Wrap(
                spacing: 8,
                children: _post.topics.map((topic) => GestureDetector(
            ...
            Padding(
              ...
              child: Row(
                children: [
                  IconButton(icon: const Icon(Icons.thumb_up), onPressed: _likePost),
                  Text('${_post.likes}'),
                ...
                children: [
                  ...
                  ..._comments.map((comment) => ListTile(
                    ...
                    subtitle: Column(
                      ...
                      children: [
                        Text(comment['text']),
                        Text(timeago.format(comment['time'], locale: 'vi'), style: TextStyle(color: Colors.grey.shade600)),
                      ],
                    ),
                    trailing: IconButton(
                      icon: const Icon(Icons.thumb_up_outlined),
                      onPressed: () => _markHelpful(_comments.indexOf(comment)),
                    ),
                  )).toList(),
                  TextField(
