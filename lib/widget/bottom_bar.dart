...
import 'package:language_app/Task1/home_screen.dart'; // Import màn hình chính
import 'package:language_app/Task1/vocabulary_topic_screen.dart'; // Import màn hình chủ đề từ vựng
...
class Bottombar extends StatefulWidget {
  const Bottombar({super.key, required this.type}); // Constructor với tham số type để xác định nút nào được chọn
  final int type; // Biến type xác định nút đang active (1-5)

  @override
  State<Bottombar> createState() => _BottombarState(); // Tạo state cho Bottombar
}
...
class _BottombarState extends State<Bottombar> {
  ...
  Widget build(BuildContext context) {
    ...
    final pix = size.width / 375; // Tỷ lệ pixel dựa trên chiều rộng chuẩn 375px (để responsive)
    return Container(
      height: 64 * pix, // Chiều cao thanh bottom bar, tính theo tỷ lệ
      width: 312 * pix, // Chiều rộng thanh, tính theo tỷ lệ
      padding: EdgeInsets.symmetric(horizontal: 8 * pix), // Khoảng cách lề trái/phải
      ...
      child: Row(
        ...
        children: [
          ...
          _buildActionButton(
              ...
              enabled: widget.type == 5 ? true : false), // Active nếu type = 5
        ],
      ),
    );
  }
  ...
  Widget _buildActionButton({
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: enabled ? AppColors.primary : Colors.transparent, // Màu primary nếu active, trong suốt nếu không
          borderRadius: BorderRadius.circular(50), // Bo tròn nút
        ),
        child: Center(
          child: Image.asset(image, width: 32 * pix, height: 32 * pix), // Hiển thị icon, kích thước theo tỷ lệ
        ),
      ),
    );
  }
